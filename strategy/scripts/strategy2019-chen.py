...
class Strategy(NodeHandle):
	def __init__(self):
		...
		self.ballarea = 0
		# test
		self.coordinate = [[0.0,0.0],[0.0,0.0]]
...
#================================
	def behavior_to_strings(self, argument):
		switcher = {
			...
			1: "BEGINER",
			2: "FIND_BALL",
			3: "CATCH_BALL",  #positive
			4: "GOAL",
			5: "FIND_BALL2",  #reverse
			6: "SHORT_SHOOT",
			7: "FAR_SHOOT"
		}
	...
	def z_speed_planning(self, angle):#Z linear speed conversion
	...
	def Find_Ball_Strategy(self):
		if(self.state == 0):
			if(self.lostball == False):
				...
				self.balldis = 999
				self.ballang = 999
			RPang = Norm_Angle((90 + self.findang) - self._front) if(self._front >= 0) else Norm_Angle((-90 + self.findang) - self._front)
			if(abs(RPang) > self.error_ang):
				x = 0
	...
	def Find_Ball_Strategy2(self):
		if(self.state == 0):
			if(self.lostball == False):
				...
				self.balldis = 999
				self.ballang = 999
			buffer = self.findang - self._front if(self._front >= 0) else -self.findang - self._front
			RPang = Norm_Angle(buffer)
			if(abs(RPang) > self.error_ang):
				x = 0
				z = self.find_ball_vel_z if(RPang > 0) else -self.find_ball_vel_z
				self.Robot_Vel([x, z])
	...
	def Process(self):
		...
		if(self._start):
			...
			self.behavior = INIT
#==================================
	def test_pos(self,target=[0.3, -0.3]):
		self.goal=target

		buffer = [self.goal[0] - self._pos[0], self.goal[1] - self._pos[1]]
		print("self._pos","%.2f" % self._pos[0],"%.2f" % self._pos[1],"buffer",buffer)
		print("")
		print("Get_Dis",self.Get_RP_Dis(buffer),"Get_Ang",self.Get_RP_Angle(self.goal))
		RPdis = self.Get_RP_Dis(buffer)
		RPang = Norm_Angle(self.Get_RP_Angle(self.goal) - self._front)
		x = self.x_speed_planning2(RPdis)
		z = self.z_speed_planning2(RPang)
		print("x",x,"z",z)
		self.Robot_Vel([x, z])
		if(x == 0 and z == 0):
			print("go to next")
	...
	def test_set(self,target=[0.5,-0.5]):
		self.goal =target
		self.behavior =FIND_BALL2
	def test_pos2(self):
		print("test pos2")
		RPdis = self.Get_RP_Dis(self.goal)
		RPang = Norm_Angle(self.Get_RP_Angle(self.goal) - self._front)
		print("RPang",RPang,"ang",self.Get_RP_Angle(self.goal),"_front",self._front)
		if(abs(RPang) < self.error_ang):
			print("ang_!!!!",abs(RPang) > self.error_ang)
		if(RPdis > self.error_dis):
			if(self.prev_RPdis >= RPdis):
				x = self.x_speed_planning(RPdis)
				z = self.z_speed_planning(RPang) if(abs(RPang) > self.error_ang) else 0
	...
	def Process2(self):
		...
		if(self._start):
			...
			switcher = {
				INIT : self.Init_Strategy,
				# BEGINER : self.Beginer_Strategy
				# BEGINER : self.test_pos
				BEGINER : self.test_set,
				FIND_BALL2 : self.test_pos2
			}
			run_Process = switcher.get(self.behavior,self.Robot_Stop)
			run_Process()
		else:
	...
	def x_speed_planning2(self, distance):#X linear speed conversion
		...
		# if(distance != 0):
			...
			planning_speed = 0
		return planning_speed
	def z_speed_planning2(self, angle):#Z linear speed conversion
