...
class Strategy(NodeHandle):
	...
	def Process(self):
		if(self._start):
			print(self.state,self.behavior,self.ballcolor,self.ballarea,self.ballang)
		...
		if(self.state == 0):
			...
			for i in range(len(self._ballsColor)):
				if(self._ballsColor[i] == self.ballcolor):
					color = self._ballsColor[i]
					break
			for j in range(len(self._ballsColor2)):
				if(self._ballsColor2[j] == self.ballcolor):
					color2 = self._ballsColor2[j]
					break
			if(color2):
				RPdis = self._ballsCatch[j]
				if(RPdis > 100):
					self.behavior = GOAL
					self.goal = self._goalarea[color2]
					self.state = 0
					self.Robot_Stop() 
				else:
					x = self.vel_x-self.vel_z
	...
	def	Goal_Strategy(self):
		if(self.state == 0):
			color = None
			for i in range(len(self._ballsColor2)):
				if(self._ballsColor2[i] == self.ballcolor):
					color = self._ballsColor2[i]
					break
			if(self.state == 0):
				RPang = Norm_Angle(self.Get_RP_Angle(self._goalarea[self.ballcolor])-self._front)
				#RPang = Norm_Angle(self.Get_RP_Angle(self.goal)-self._front)
			...
			RPang = Norm_Angle(self.Get_RP_Angle([self.findballpos[0],self.goal[1]])-self._front)
